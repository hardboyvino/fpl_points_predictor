...
for index, row in dataset.iterrows():
    ...
    if budget >= 0 and len(team) < 12:
        ...
        if budget >= row['Prices'] and row["Prices"] <= 16.5 and positions[row['Position1']] != 0 and row['Player1'] not in team and row['Player1'] not in exclusion:
            ...
            if positions[row['Position2']] != 0 and row['Player2'] not in team and row['Player2'] not in exclusion:
                ...
                if (new_position[row['Position1']] <=  positions[row['Position1']]) and (new_position[row['Position2']] <=  positions[row['Position2']]) and (epl_team[row['Team1']] <= max_from_teams[row['Team1']]) and (epl_team[row['Team2']] <= max_from_teams[row['Team2']]):
